"""Custom grid-search scripts to select the best parameter in features extraction,
...
def perform_reduction_exp(params):
    ...
    try:
        ...
        # Set the model
        training_set, validate_set = \
            train_test_split(dataset["train"], test_size=preprocessing.VALIDATE_PART, random_state=seed)
        target_training = preprocessing.create_target(training_set)
        target_validate = preprocessing.create_target(validate_set)
        ...
        # Compute scores
        score_training = preprocessing.loss_fct(target_training, pred_training)
        score_validate = preprocessing.loss_fct(target_validate, pred_validate)
...
def perform_ct_exp(params):
    ...
    try:
        ...
        ft_extractor = preprocessing.create_ft_ct(ngram=n, min_df=min_df, max_df=max_df)
...
def perform_ct_pd_au_exp(params):
    ...
    try:
        ...
        ft_extractor = preprocessing.create_ft_ct_pd_au(w_ct=w_ct, w_pd=w_pd, w_au=w_au)
        classifier = LogisticRegression(verbose=0)
        pipe = Pipeline([('ft_extractor', ft_extractor), ('classifier', classifier)])
...
def plot_ct_pd_au(filename):
    ...
    results = utils.load_pickle(filename)
    arg_min = np.argmin(np.array([r[5] for r in results]))
    print results[arg_min]
